...
export default function Register() {
  ...
  const onSubmit = async () => {
    ...
    if (!fullName.trim() || !email.trim() || !password) {
      setError('Renseigne ton nom, ton email et un mot de passe.');
      return;
    }
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Text style={[styles.title, { color: c.txt }]}>Crée ton compte</Text>
        <Text style={[styles.subtitle, { color: c.muted }]}>
          Commence ton parcours avec un coach IA.
        </Text>
        ...
        <View style={styles.switchRow}>
          <Text style={[styles.switchTxt, { color: c.muted }]}>Déjà un compte ?</Text>
          <PressableScale onPress={() => router.replace('/(auth)/login')} hitSlop={8}>
            <Text style={[styles.switchLink, { color: c.accent }]}>Se connecter</Text>
          </PressableScale>
        </View>
      </ScrollView>
    </KeyboardAvoidingView>
...
function errorMessage(e: unknown): string {
  if (e instanceof AxiosError) {
    ...
    if (e.response?.status === 400 && data) {
      // Remonte le premier message de validation du backend (email déjà pris, etc.).
